...
def test_read_your_writes(s1, s2, cl, trials=30):
    ...
    for i in range(trials):
        ...
        def reader():
            ...
            except (Unavailable, ReadTimeout, WriteTimeout, OperationTimedOut):
                res['failed'] = True
...
# =========================================================
# 2. MONOTONIC READS (Uses 3 Distinct Nodes)
# =========================================================
def test_monotonic_reads(s1, s2, s3, cl, trials=30):
    ...
    for i in range(trials):
        ...
        def writer_and_reader1():
            try:
                s1.execute(SimpleStatement(f"INSERT INTO events (id, value) VALUES ({key}, {val})", consistency_level=cl))
                row1 = s2.execute(SimpleStatement(f"SELECT value FROM events WHERE id = {key}", consistency_level=cl)).one()
        ...
        def reader2():
            barrier.wait()
            try:
                row2 = s3.execute(SimpleStatement(f"SELECT value FROM events WHERE id = {key}", consistency_level=cl)).one()
...
# =========================================================
# 3. WRITES FOLLOW READS (Uses 3 Distinct Nodes)
# =========================================================
def test_writes_follow_reads(s1, s2, s3, cl, trials=30):
    ...
    for i in range(trials):
        ...
        try:
            s1.execute(SimpleStatement(f"INSERT INTO events (id, value) VALUES ({key}, {v_initial})", consistency_level=ConsistencyLevel.ALL))
        except Exception:
            pass
        ...
        def client_b_causal_write():
            ...
            try:
                ...
                if row and row.value == v_initial:
                    s3.execute(SimpleStatement(f"INSERT INTO events (id, value) VALUES ({key}, 999)", consistency_level=cl))
            except (Unavailable, ReadTimeout, WriteTimeout, OperationTimedOut):
                res['failed'] = True
...
# =========================================================
# 4. MONOTONIC WRITES (Uses 3 Distinct Nodes)
# =========================================================
def test_monotonic_writes(s1, s2, s3, cl, trials=30):
    ...
    for i in range(trials):
        ...
        try:
            final = s3.execute(SimpleStatement(f"SELECT value FROM events WHERE id = {key}", consistency_level=ConsistencyLevel.ONE)).one()
